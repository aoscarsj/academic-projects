...
public class TelaQuestMultEsc extends JFrame implements ActionListener{
	...
	private ButtonGroup bgDificuldade;
	...
	QuestoesMulti questao = new QuestoesMulti();
	...
	private void setVar (){
		...
		lbTipo = new JLabel ("Questões de Múltipla Escolha");
		lbTipo.setFont(new Font("DialogInput", Font.BOLD + Font.ITALIC + Font.PLAIN, 40));
		
		lbProfessor = new JLabel ("Professor: ");
		lbProfessor.setFont(new Font("Serif", Font.ITALIC + Font.PLAIN, 15));
		lbMateria = new JLabel ("Matéria: ");
		lbMateria.setFont(new Font("Serif", Font.ITALIC + Font.PLAIN, 15));
		...
		lbInstrucoes2 = new JLabel("2. *Em 'Respostas' digite em cada caixa quais serão as alternativas e assinale as alternativas corretas.");
		...
		tfProfessor.setFont(new Font("SansSerif", + Font.BOLD + Font.PLAIN, 14));
		tfProfessor.setText("Alexandre Franco");
		tfProfessor.setEditable(false);
		tfMateria = new JTextField (10);
		tfMateria.setFont(new Font("SansSerif", Font.BOLD + Font.PLAIN, 14));
		...
		tfQuestao.setFont(new Font("Serif",  Font.ITALIC, 14));
		tfA.setFont(new Font("Serif",  Font.BOLD, 14));
		tfB.setFont(new Font("Serif",  Font.BOLD, 14));
		tfC.setFont(new Font("Serif",  Font.BOLD, 14));
		tfD.setFont(new Font("Serif",  Font.BOLD, 14));
		tfE.setFont(new Font("Serif",  Font.BOLD, 14));
	...
	public void actionPerformed(ActionEvent event) {
		...
		boolean testeRBA = ( (rbA.isSelected() && (rbB.isSelected() || rbC.isSelected() || rbD.isSelected() || rbE.isSelected())) || (rbB.isSelected() && (rbC.isSelected() || rbD.isSelected() || rbE.isSelected())) || (rbC.isSelected() && (rbD.isSelected() || rbE.isSelected())) || (rbD.isSelected() && rbE.isSelected())  );
		if (event.getSource() == btCancelar)
			...
			if(testeTF || !testeRBD || !testeRBA){
				if(testeTF)
					...
					JOptionPane.showMessageDialog(null, "Lembre-se que deve assinalar +1 alternativa correta.", "Atenção", 2);
	...
	private void setCor(){
		...
		tfA.setBackground(Color.lightGray);
		tfB.setBackground(Color.lightGray);
		tfC.setBackground(Color.lightGray);
		tfD.setBackground(Color.lightGray);
		tfE.setBackground(Color.lightGray);
	...
	private void setLayout(){
		...
		int dificuldade = 0;
		Alternativas[0] = tfA.getText();
		Alternativas[1] = tfB.getText();
		Alternativas[2] = tfC.getText();
		Alternativas[3] = tfD.getText();
		Alternativas[4] = tfE.getText();
		if(rbD1.isSelected())
			dificuldade = 1;
		else if(rbD2.isSelected())
			dificuldade = 2;
		else if(rbD3.isSelected())
			dificuldade = 3;
		else if (rbD4.isSelected())
			dificuldade = 4;
		else if (rbD5.isSelected())
			dificuldade = 5;
